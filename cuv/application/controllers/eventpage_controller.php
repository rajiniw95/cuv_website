...
class eventpage_controller extends CI_Controller{
    ...
    function index(){
        ...
        $query = $this->Eventpage_model->get_event();
        $data['EVENT'] = null;
        if($query){
   			$data['EVENT'] =  $query;
  		}
        ...
        $query1 = $this->Eventpage_model->get_participants();
 	...
 	function going(){
 		$query = $this->Eventpage_model->update_going();
 		redirect('http://localhost/cuv_website/cuv/event');
 	}

 	function notgoing(){
 		$query = $this->Eventpage_model->update_notgoing();
 		redirect('http://localhost/cuv_website/cuv/event');
 		}
